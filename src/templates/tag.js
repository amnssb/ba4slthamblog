...
import { escapeHtml, formatDate, slugify, truncate, withBasePath } from '../lib/utils.js';
...
export function renderAllTags(config, tagMap, theme = 'anime-sakura') {
  ...
  const tagsHtml = Object.entries(tagMap)
    ...
    .map(([tag, posts]) => {
      ...
      return `
    <a href="${withBasePath(`/tag/${pathTag}/`, basePath)}" class="tag tag-large">
      ${escapeHtml(tag)}
